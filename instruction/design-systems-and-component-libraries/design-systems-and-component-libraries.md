...
*   **Overly consistent:** Using the popular css or web frameworks reduces user cognitive load, but it can also make your application forgettable as it blends in with the crowd.
    *   *Solution:* Embrace consistency but look for ways to distinguish yourself. This can be superior content presentation and navigation or pushing some patterns to an extreme such as when Google moved to extreme use of whitespace on their homepage.
